...
% !TEX root =  ../output/2024_Computing_Expo_-_Project_Brochure.tex
% !TEX root =  ../output/2024_Computing_Expo_-_List_of_Projects.tex

% TEX root = ../student_content/Binu_Peter_46FEC.tex
% TEX root =  ../output/BSc_H_in_Creative_Computing.tex
% !TEX root =  ../output/2024_Computing_Expo_-_List_of_Projects.tex
...
\fancyhead[LO]{\hyperlink{s:toc}{2024 Computing Graduate Handbook}}
...
\newcommand\ProgrammeLink[1]{
	...
	\hyperlink{#1}{%
		...
		\parbox{15pt}{\hfill~\pageref{c:#1}}}%\hspace*{-13pt}
 	}\par\vspace{6pt}
}
...
\newcommand\AreaLink[1]{
	\par\noindent%
	\hyperlink{#1}{%
		\parbox[t]{\textwidth}{{\bfseries \pgfkeysvalueof{/area/#1/Name}}~\dotfill~%
		 \pgfkeysvalueof{/area/#1/Count}\raisebox{-2pt}{\parbox{1cm}{\dotfill}}~%
		 \parbox{15pt}{\hfill~\pageref{area:#1}}}%\hspace*{-13pt}
 	}\par\ifbool{isSummary}{\vspace{4pt}}{\vspace{2pt}}
}{
...
\newcommand{\areastoc}{%
	...
	{\typeout{Warning: labels may have changed. Rerun}}
}
...
\newcommand\typesetAcademicTitle{{%
	...
	\color{studentTitle}\bfseries\large \parbox{20cm}{\raggedright\AcademicTitle}\par}}
...
\newcommand\typesetTechnologies{%
	...
	\ifdefempty{\TMP}{}{%
		...
		{\bfseries Technologies:} \pgfkeysvalueof{/students/\Key/Technologies}
		}
	}	
}
...
\newcommand\typesetLocation{%
	...
	\ifbool{isSummary}{%
		\begin{tikzpicture}
  			\node[draw, fill=studentheading!10, minimum width=3.7cm] (box) at (0,0) 
				{%
...
\newcommand\typesetSupervisor{%
	...
	\ifdefempty{\TMP}{}{%
		\ifbool{isSummary}{%
			\parbox[t]{2.6cm}{\bfseries \pgfkeysvalueof{/students/\Key/SupervisorLabel}:}
			\parbox[t]{14.8cm}{\pgfkeysvalueof{/students/\Key/Supervisor}}
		}{%	
		}
	}	
}
...
\tikzset{%
 	DEBUG/.style = {},
	COMMERICAL_TITLE/.style = {DEBUG,anchor=north west, text width=16.5cm,text height=12pt, text depth=3pt},
	...
	ACADEMIC_TITLE/.style = {CONTENT,text width=18cm},
...
\begin{tikzpicture}
	...
	\node[COMMERICAL_TITLE] (COMMERICAL_TITLE) {%
		...
		\parbox{16.5cm}{\hfill\CommercialTitle\hfill\mbox{}}\par%
	};
	
	\node[anchor=north west,scale=1.4] at ($(COMMERICAL_TITLE.north west)+(16.5,0.2)$) {\typesetLocation};
	...
	\node[ACADEMIC_TITLE] (ACADEMIC_TITLE) 
		...
		{%
			\pgfkeysgetvalue{/students/\Key/AcademicTitle}{\AcademicTitle}%
			\color{studentTitle}\bfseries\large%
			\parbox{18cm}{\raggedright\AcademicTitle}\par%
		};
	\node[NAME] (NAME) at ($(ACADEMIC_TITLE.south east)+(2,0)$)
		{\typesetName};		

	\node[LABEL] (PROJECT_AREAS_LABEL) 
		at ($(ACADEMIC_TITLE.south west) + (-1,-0.2)$)
		{Project Areas};
	\node[CONTENT,text width=11cm] (PROJECT_AREAS) 
	...
	\ifdefempty{\TMP}
		...
		{%
			...
			\node[CONTENT,text width=8cm] (SUPERVISOR) 
	...
	% poster + summary 	
	\ifbool{isPosterLandscape}{%
		\node[POSTER,anchor=north west,text width=13.5cm] (POSTER) 
			at ($(PHOTO.north west)+(0,-6.7)$)
			{\typesetPoster};
		\node[SUMMARY,anchor=north east, text width=9cm] (SUMMARY) 
			at ($(NAME.south east)+(0,-2)$)
			{\Summary};
		\node[anchor=north east] at ($(SUMMARY.south east)+(0,-1)$)
			{\parbox[t]{2.5cm}{\bf Technologies: }\ \parbox[t]{7cm}{\Technologies}};
		\node[PROJECT_URL,anchor=north east] (PROJECT_URL) 
			at ($(COMMERICAL_TITLE.north east)+(5,-15.5)$) 
			{\typesetQR};
		\node[PROJECT_URL,anchor=north east,fill=white,text width=23.5cm] (PROJECT_URL) 
		at ($(COMMERICAL_TITLE.north east)+(2.5,-17)$) 
			{\typesetProjectURL};

	}{% portrait 
		\node[POSTER, text width=10cm] (POSTER) 
			at (NAME.south east)
			{\typesetPoster};
		\node[SUMMARY,anchor=north west, text width=14cm] (SUMMARY) 
			at ($(PHOTO.south west)+(0,-2.5)$)
			{\def\parskip{12pt}\Summary\par};
		\node[anchor=north west] at ($(SUMMARY.south west)+(0,-1)$)
			{\parbox[t]{2.5cm}{\bf Technologies: }\ \parbox[t]{10cm}{\Technologies}};

		\node[PROJECT_URL,anchor=north west] (PROJECT_URL) 
			at ($(PHOTO.north west)+(-0.5,-15.5)$) 
			{\typesetQR};
		\node[PROJECT_URL,anchor=north west] (PROJECT_URL) 
		at ($(PHOTO.north west)+(2.5,-17)$) 
			{\typesetProjectURL};
	}
...
\vskip 48pt plus 48pt minus 50pt
\vskip \ENTRYSKIP

\begin{tikzpicture}
	%\node[LABEL,align=center] (L1) {Commercial Title:};
	\node[TITLE,text width=17cm,align=center, text depth=2pt] %at ($(L1.south west)+(1,0.1)$) 
		(C_TITLE) 
		{\hypertarget{\Key}{}\label{project:\Key}\writeprojectlink\typesetCommercialTitle};
	\node[LABEL] at ($(C_TITLE.south west)+(0,-0.2)$) (L2) {Academic Title:};
	\node[TITLE,anchor=north west,text width=20cm]  at ($(L2.south west)+(1,0.1)$)  (A_TITLE) 
		{\hypertarget{\Key}{}\label{project:\Key}\writeprojectlink\typesetAcademicTitle};
	\node[NAME] at (A_TITLE.south east) (NAME) {\typesetName};
	\node[PHOTO,anchor=north east] at ($(C_TITLE.north west) +(-1,0)$) (PHOTO) {\typesetPhoto};
%	\node[SUMMARY,text width=20cm] at ($(NAME.south east)+(0,0)$) (SUMMARY) {\typesetSummary};
	\node[TECHNOLOGIES,text width=17.5cm] at ($(NAME.south east)+(-20.2,-0.3)$) (TECHNOLOGIES) 
			{\typesetTechnologies};
	\node[PROJECT_AREAS,text width=17.5cm] at ($(TECHNOLOGIES.south west)+(0,-0.1)$) (PROJECT_AREAS) 
			{\typesetProjectAreas};
	\node[PROJECT_AREAS,text width=17.5cm,anchor=north west] at ($(PROJECT_AREAS.south west)+(0,-0.3)$) (SUPERVISORS) 
			{\typesetSupervisor};
	\node[PROJECT_URL,text width=20cm,anchor=north west] at ($(SUPERVISORS.south west)+(-0.1,-0.1)$) (PROJECT_URL) 
				{\typesetProjectURL};
	\node[anchor=north east] at ($(NAME.south east)+(0,0.)$) (QR) 
				{\typesetQR};

	\node[anchor=north west] at ($(C_TITLE.north west)+(16.3,0.05)$) {\typesetLocation};
\end{tikzpicture}}
